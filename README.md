...
<script>
  const passwordCorrect = "ZCVPLAPL1";
  ...
  function checkPassword() {
    ...
    if (input === passwordCorrect) {
      ...
      alert("Sai Key Vui Lòng Nhập Lại🚫!");
    }
  }
  ...
  function chooseApp() {
    ...
    if (option) {
      ...
      if (file) {
        alert("Đã chọn tệp: " + file.name);
      }
      document.getElementById('appSelect').classList.remove('hidden');
    } else {
